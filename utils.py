...
class linkpred_metrics():
    ...
    def get_linkpred_metric(self, emb):
        # if emb is None:
...
def load_data2(dataset):
    ...
    rootdir = './data/'
    try:
        data_c = h5py.File(rootdir+filename+'/'+filename +
                           '_content_n.mat')['content'][:].T
    except BaseException:
        data_c = sio.loadmat(rootdir+filename+'/' +
                             filename+'_content_n.mat')['content']
    #data_c = h5py.File(rootdir+filename+'/'+filename+'_content_n.mat')['content'][:].T
    data_t = h5py.File(rootdir+filename+'/'+filename+'_n.mat')['G'][:].T
    data_l = h5py.File(rootdir+filename+'/'+filename +
                       '_rlabels_n.mat')['labels'][:].T
    print(data_t.shape)
    if dataset == 'pubmed':
        data_c = data_c.T
