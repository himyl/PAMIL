...
from utils import load_data, load_data2, preprocess_graph
...
def gae_for(args):
    ...
    for epoch in range(args.epochs):
        ...
        if int(args.M) > 0:
            ...
            for j in range(M):
                ...
                D_xi_new_0 = D2(X_hat)
                D_xi_new_1 = D2(features)
                D_xi_new = torch.concat([D_xi_new_1, D_xi_new_0], axis=0).to(device)
                D_xi_new_mat = d2_output_mat(D_xi_new).to(device)
                D_xi_old_mat = d2_output_mat(D_xi_old).to(device)
                kl = F.kl_div(D_xi_new_mat, D_xi_old_mat,
                            reduction='batchmean', log_target=True)
    ...
    df_ = df_.loc[:, ["AC", "NMI", "F1", "Precision", "ARI"]]
...
if __name__ == '__main__':
    ...
    for lr in [0.001]:
        ...
        for p_alpha in [0.001, 0.01, 0.1, 0.3, 0.9, 1.5, 5, 20]:
            num_exp = 10
            for _ in range(1, num_exp+1):
                ...
                if 1:
                    ...
                    fh.write('lr = {}, alpha = {}, M = {}, id = {}, Acc = {}, NMI = {}, ARI = {}'.format(
                        args.lr, args.p_alpha, args.M, _, metric["AC"], metric["NMI"], metric["ARI"]))
                    fh.write('\r\n')
                    fh.flush()
                    fh.close()
                # save best embedding
